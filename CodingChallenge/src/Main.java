...
public class Main {
    ...
    private void run() {
        TransactionRecord.readTransactionsFromCSV( FilePath );
    }
    ...
    private void checkTransactions(String accountId, LocalDateTime from, LocalDateTime to) {
        ...
        double transferFromAccBal = 0.0d;
        double transferToAccBal = 0.0d;
            ...
            if ((t.getCreateAt().isAfter( from ) || (t.getCreateAt().isEqual( from ))) && ((t.getCreateAt().isBefore( to ) || t.getCreateAt().isEqual( to )))) {
                if ((t.getFromAccountId().contains( accountId ))) {
                    ...
                    if ((t.getTransactionType().contains( "PAYMENT" ))) {
                        t.setAmount( transferFromAccBal - t.getAmount() );
                        transferFromAccBal = t.getAmount();
                    }
                }
                if (t.getToAccountId().contains( accountId )) {
                    numOfTran++;
                    if ((t.getTransactionType().contains( "PAYMENT" ))) {
                        t.setAmount( transferToAccBal + t.getAmount() );
                        transferToAccBal = t.getAmount();
                    }
                }
            }
        }
        finalAccBalance = transferFromAccBal + transferToAccBal;
        System.out.println( "Relative transferFromAccBal for the period is: " + NumberFormat.getCurrencyInstance().format( finalAccBalance ) );
